...
class PGAgent(nn.Module):
    ...
    def update(
        ...
        if self.critic is not None:
            ...
            for i in range(self.baseline_gradient_steps):
                baseline_loss = self.critic.update(obs, q_values)
    ...
    def _estimate_advantage(
        ...
        if self.critic is None:
            ...
            # print("VALUE AND Q SHAPE", values.shape, q_values.shape)
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        for reward in rewards:
            ...
            sum_discounted_reward = np.sum(gamma * reward)
            discounted_reward = np.repeat(sum_discounted_reward, T)
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        for reward in rewards:
            T = len(reward)
            discounted_reward = np.zeros(T, dtype=np.float32)
            gamma = self.gamma ** np.arange(0, T)
            for t in range(T):
                discounted_reward[t] = np.sum(
                    gamma[0: T - t] * reward[t:]
                )
            discounted_rewards.append(discounted_reward)
