...
class IQLAgent(AWACAgent):
    ...
    def update_q(
        ...
        with torch.no_grad():
            next_target_values = self.target_value_critic(next_observations).squeeze(1)
            target_values = rewards + self.discount * (1 - dones.float()) * next_target_values
    ...
    def iql_expectile_loss(
        ...
        weight = torch.where(target_qs - vs <= 0, 1 - expectile, expectile)
        return weight * ((target_qs - vs) ** 2)
    ...
    def update_v(
        ...
        with torch.no_grad():
            target_values = torch.gather(
                self.target_critic(observations),
                dim=1,
                index=actions.unsqueeze(1),
            ).squeeze(1)
